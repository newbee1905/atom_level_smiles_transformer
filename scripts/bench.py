...
def main(cfg: DictConfig):
	...
	if cfg.get("bench_steps", 100) < 1:
		print("Error: 'bench_steps' must be at least 1.")
		return
	...
	print(f"DataLoader created with {cfg.training.num_workers} workers.")
	...
	print(f"\nStarting benchmark: {warmup_steps} warmup steps, {num_steps} benchmark steps.")
	...
	for i in range(warmup_steps + num_steps):
		...
		# Store timings after warmup phase
		if i >= warmup_steps:
		...
		if (i + 1) % 10 == 0:
			print(f"  Step {i + 1}/{warmup_steps + num_steps} completed...")
